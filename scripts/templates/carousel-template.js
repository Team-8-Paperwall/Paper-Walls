...
const carouselTemplate = function (category) {
    ...
    const generateCarouselItem = function (imageSrc, comments) {
        let caroussel = $("<div>");
        caroussel.addClass("item");
        let carousselImage = $("<div>");
        carousselImage.addClass("col-lg-12 col-md-12 col-sm-12 col-xs-12 carouselImage");
        carousselImage.append($("<img>").attr("src", imageSrc));
        caroussel.append(carousselImage);
        let commentsDiv = $("<div>");
        commentsDiv.addClass("pictureComments");
    ...
    category.forEach(function (el) {
        let carousselItem = generateCarouselItem(el.location, el.comments);
